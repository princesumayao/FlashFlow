...
<div class="flex flex-col p-8 bg-gradient-to-br from-zinc-900 via-zinc-800 to-black rounded-2xl border border-zinc-700 shadow-xl group transition-all duration-300">
    <div class="flex justify-between items-start">
        <div>
            <h3 class="text-2xl font-extrabold group-hover:text-blue-500 transition-colors duration-300">{{ $title }}</h3>
    ...
    <div class="mt-4 flex items-center justify-between">
        <span class="text-xs text-white/40">{{ $postedDate }}</span>
        <div class="flex gap-2">
            ...
            <form action="{{ $deleteUrl }}" method="POST" onsubmit="return confirm('Delete this job?');" class="inline-block">
                @csrf
                @method('DELETE')
                <button type="submit"
