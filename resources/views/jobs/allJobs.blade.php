<x-layout>
    <div class="relative">
        ...
        <section class="pt-4 max-w-5xl mx-auto relative z-10 px-4 md:px-0 -mt-10">
            <div class="mb-8">
                <a href="{{ url()->previous() }}" class="inline-flex items-center gap-2 text-white/70 hover:text-white transition text-sm md:text-base">
                ...
                <h1 class="font-bold text-2xl md:text-4xl mt-4">All Jobs from {{ $employer->company_name }}</h1>
            </div>
            ...
            <div class="space-y-6">
                @forelse($jobs as $job)
                    <x-job-card :$job />
                @empty
                    <p class="text-white/60 text-sm md:text-base">No jobs posted yet</p>
                @endforelse
            </div>
        </section>
    </div>
